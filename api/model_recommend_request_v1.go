...
// RecommendRequestV1 Body of `POST /api/v1/drivers`. Mirrors the upstream Recommend service contract. Note that the version field is named `version` (not `pipeline_version` as on `/forecasts`).  Required fields: `version`, `recency_factor`, `timeseries_metadata`. Both `filters` and `timeseries` are optional on the recommend endpoint — when omitted, the handler drops them from the upstream payload entirely (no `null` is sent).  When `filters` is present, **`categories[]`** and **`regions[]`** follow the same rules as `POST /api/v1/forecasts`: each integer must be **1–9999** inclusive; optional **`limit`** is **0–1000**. See schema **`Filters`** for the canonical shape. 
type RecommendRequestV1 struct {
	// Optional. Same validation as **`POST /api/v1/forecasts`**: each **`categories[]`** and **`regions[]`** entry is an integer **1–9999** (inclusive); optional **`limit`** is **0–1000**. Values are not verified against catalog APIs. 
